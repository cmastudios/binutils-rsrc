...
static int parse_args_type(rsrc_inst_t *output, char *str, enum rsrc_itype type, int cond)
{
	...
	switch (type)
	{
		case RSRC_1_A_B_C2:
			...
			if (output->form2.op == RSRC_ADDI || output->form2.op == RSRC_ANDI || output->form2.op == RSRC_ORI)
			{
				if (sscanf(str + 1, "r%d, %d", &rb, &c2) < 2) {
					as_bad("Not enough arguments (expected ra, rb, c2)");
					return -1;
				}
				output->form2.c2 = c2;
				output->form2.rb = rb;
			}
			else if (sscanf(str + 1, "%d(r%d)", &c2, &rb) == 2)
			{
				output->form2.c2 = c2;
				output->form2.rb = rb;
			}
			else if (sscanf(str + 1, "(r%d)", &rb) == 1)
			{
				output->form2.c2 = 0;
				output->form2.rb = rb;
			}
...
arelent * tc_gen_reloc(asection *section ATTRIBUTE_UNUSED, fixS *fixp)
{
	...
	*rel->sym_ptr_ptr = symbol_get_bfdsym(fixp->fx_addsy);
	...
	if (!rel->howto)
	{
		...
		if (!rel->howto)
			as_bad_where(fixp->fx_file, fixp->fx_line, "ERROR: rel->howto");
	}
